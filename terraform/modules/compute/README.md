...
**⚙️ AMI로 API 인스턴스를 실행하고 확장·교체하는 구성**

시작 템플릿에는 API 서버의 실행 환경을 정의합니다. Auto Scaling Group은 이 환경에서 실행할 인스턴스를 관리합니다. 서버를 두 가용 영역의 프라이빗 서브넷에 배치하며 CPU 부하에 따라 처리 용량을 조절합니다.
