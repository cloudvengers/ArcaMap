...
운영 중 발생한 로그·지표·이벤트·추적 기록인 **텔레메트리**의 수집 경로는 다음과 같습니다.
...
**ALB·CloudFront 접근 로그와 시스템 그룹의 API 기록은 있지만 업무 기능의 성공 여부를 연결할 자료는 부족합니다.** 공통 기간의 서울 CloudWatch Logs와 CloudFront 로그용 S3에 남은 기록은 다음과 같습니다.
...
**공통 기간의 CPU 부하는 낮지만 한 AZ만 남았을 때의 처리 용량은 시험 근거가 없습니다.** 서울 API 인스턴스·ASG와 RDS `arcamap-postgres`의 CloudWatch 지표는 다음과 같습니다.
